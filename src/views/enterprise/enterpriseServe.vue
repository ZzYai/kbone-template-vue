...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      //获取全部标签
      this.$api.enterprise.getClassify({ enterprise: this.$route.query.id, type: 2 }).then((resp) => {
        if (resp) {
    ...
    serveDetail(id, item) {
      ...
      // this.$store.commit("enterprise/setEnterpriseServeData", item)
    }
  }
...
.enterpriseServe .van-dropdown-menu__bar {
  height: 90px;
}
.enterpriseServe .van-ellipsis {
  font-size: 30px;
  line-height: 80px;
}
.enterpriseServe .van-cell {
  font-size: 30px;
  line-height: 100px;
  height: 100px;
}
.enterpriseServe .van-list__finished-text,
.enterpriseServe .van-list__loading {
  width: 700px;
  padding: 0;
...
.enterpriseServe .van-loading__text {
  font-size: 24px;
}
.enterpriseServe .van-dropdown-menu__title::after {
  border: 6px solid;
  border-color: transparent transparent #dcdee0 #dcdee0;
  right: -15px;
  margin-top: -7px;
}
.enterpriseServe .van-dropdown-menu__title--active::after {
