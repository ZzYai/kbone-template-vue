<template>
  <div class="login">
    ...
    <form class="formDiv" @submit.prevent="reg">
      <div
        class="formInput"
      ...
      <div class="btns">
        <button class="redBtn">确认注册</button>
      </div>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    reg() { // 登录     注册
      ...
      this.$api.user.getRegister(data).then(resp => {
        register.clear()
        if (resp) {
           Toast.success({
            duration: 2000, // 持续展示 toast
          ...
          setTimeout(() => {
            this.$store.commit("user/setLoginStatus", 4);
          }, 1500)
        }
      })
    },
